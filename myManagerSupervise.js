...
function searchConditions(flag) {
    ...
    if (flag == "simple") {
        ...
        o.serialNumber = $("#serialNumber").val();   //流水号
        o.processTemplateName = $("#processTemplateName").val();    //流程名称
        o.operateBeginDate = $("#operateBeginDate").val();  //经办开始时间
        o.operateEndDate = $("#operateEndDate").val();  //经办结束时间
        o.operator = $("#operator").val();   //经办人
...
function validate() {
    try {
        ...
        var operateBeginDate = $("#operateBeginDate").val();
        var operateEndDate = $("#operateEndDate").val();
        if (operateBeginDate === "" || operateEndDate === "") {

        } else {
            //OA-68717
            var strDate = operateBeginDate.replace(/-/g, "/");
            var begin = new Date(strDate);
            strDate = operateEndDate.replace(/-/g, "/");
            var end = new Date(strDate);
            if (begin > end) {
                return false;
            }
